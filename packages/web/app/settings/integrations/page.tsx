...
function IntegrationsContent(): JSX.Element {
  ...
  useEffect(() => {
    ...
    const pocketToken = searchParams.get('pocketToken')
    const state = searchParams.get('state') as ImportItemState | null
    const code = searchParams.get('code')

    if (pocketToken && state && !getIntegration('POCKET')) {
      connectToPocket(pocketToken, state)
    }

    if (code && !getIntegration('NOTION')) {
      connectWithNotion(code)
    }
  }, [getIntegration, router, searchParams])
