...
import { ResizableSidebar } from '../article/ResizableSidebar'
...
import { CircleNotch, ArrowsClockwise, Copy, Check, CaretDown, CaretUp, X } from '@phosphor-icons/react'
...
type AnkiCardsSidebarProps = {
  libraryItemId: string
  isShow: boolean
  onClose: () => void
}

export function AnkiCardsSidebar(props: AnkiCardsSidebarProps): JSX.Element {
  const { libraryItemId, isShow, onClose } = props
  const { batch, isLoading, error, revalidate } = useGetAnkiCardsQuery(
    isShow ? libraryItemId : undefined,
    false
  )
  ...
  return (
    <ResizableSidebar isShow={isShow} onClose={onClose}>
      <VStack
        css={{
          height: '100%',
          width: '100%',
      ...
      >
        ...
        >
          {isLoading ? (
            ...
            <VStack css={{ gap: '15px' }}>
              <HStack css={{ justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: '8px' }}>
